...
\begin{proc}{estaIncluida}{\In s: \TLista{T}, \In t: \TLista{T}, \Out r: \bool}{}
    \pre{True}
    \post{r = true \leftrightarrow (\forall i: \ent)(0 \leq i < |s| \implicaLuego \#apariciones(s, s[i]) <= \#apariciones(t, s[i])}
\end{proc}
...
\begin{proc}{mezclarOrdenado}{\In s, t: \TLista{\ent}, \Out r: \TLista{\ent}}{}
    ...
    \post{ \\
        |r| = |s| + |t| \\
        \land (\forall i: \ent)(0 \leq i < min(|s|, |t|) \implicaLuego r[i*2] = s[i] \yLuego r[i*2 + 1] = t[i]) \\
        \land |s| > |t| \implicaLuego subseq(s, |t|, |s|) = subseq(r, |t|, |r|) \\
        \land |t| > |s| \implicaLuego subseq(t, |s|, |t|) = subseq(r, |s|, |r|) \\
    }
\end{proc}
