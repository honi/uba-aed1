...
\begin{proc}{menorCoprimo}{\In n: \ent, \Out m: \ent}{}
    \pre{n > 0}
    \post{m > 1 \land sonCoprimos(n, m) \land (\forall k: \ent)(k > 1 \land sonCoprimos(n, k) \implicaLuego m \leq k)}
\end{proc}
...
\begin{proc}{maximaDiferencia}{\In s: \TLista{\float}, \Out r: \float}{}
    \pre{|s| \geq 2}
    \post{(\exists x, y: \ent)(x \in s \land y \in s \land r = x - y \land (\nexists z, w: \ent)(z \in s \land w \in s \land r < z - w))}
\end{proc}
